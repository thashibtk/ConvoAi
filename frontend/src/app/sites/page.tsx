...
export default function SitesPage() {
  ...
  // stagger card tilt so the grid reads like pinned cards, not a uniform SaaS grid
  const tilt = (i: number) => (i % 3 === 0 ? '-rotate-1' : i % 3 === 1 ? 'rotate-1' : 'rotate-0');
    ...
    <div className="space-y-12 pb-20">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 border-b-[6px] border-black pb-8">
        <div>
          ...
          <p className="text-xl font-bold text-gray-700 flex flex-col sm:flex-row sm:items-center gap-2">
            Your AI workspace at a glance
            {userEmail && (
              <span className="inline-block bg-brutal-cyan text-black border-2 border-black px-3 py-1 text-sm brutal-shadow-sm w-fit mt-2 sm:mt-0">
                {userEmail}
              </span>
            )}
          </p>
        ...
        <button
          onClick={() => (sites.length >= 1 ? setShowLimitModal(true) : setShowModal(true))}
          className="brutal-btn bg-brutal-yellow brutal-shadow hover:bg-black hover:text-white text-lg px-8 py-4 flex items-center gap-2"
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
          NEW AGENT
        </button>
      </div>

      {/* STAT STRIP — quick glance before the grid, mirrors the agent detail page */}
      {!loading && sites.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6">
          <div className="brutal-card brutal-shadow-sm p-4 sm:p-6 bg-white rotate-1">
            <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Agents</p>
            <p className="text-3xl sm:text-4xl font-black">{sites.length}</p>
          </div>
          <div className="brutal-card brutal-shadow-sm p-4 sm:p-6 bg-white -rotate-1">
            <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Plan</p>
            <p className="text-sm sm:text-lg font-black">Tier 1</p>
          </div>
          <div className="brutal-card brutal-shadow-sm p-4 sm:p-6 bg-white rotate-1 col-span-2 sm:col-span-1">
            <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Status</p>
            <p className="text-sm sm:text-lg font-black text-brutal-green">● ALL SYSTEMS LIVE</p>
          </div>
        </div>
      )}
      ...
      {loading ? (
        <div className="flex justify-center py-24">
          <div className="text-2xl sm:text-4xl font-black uppercase animate-pulse border-4 border-black p-6 bg-white brutal-shadow">Loading Agents...</div>
        </div>
      ) : sites.length === 0 ? (
        <div className="brutal-card brutal-shadow p-10 sm:p-16 text-center bg-white border-[6px]">
          <div className="w-20 h-20 sm:w-24 sm:h-24 bg-brutal-yellow border-4 border-black flex items-center justify-center mx-auto mb-8 brutal-shadow-sm rotate-3">
          ...
          <h3 className="text-3xl sm:text-4xl font-black uppercase mb-4">No agents yet</h3>
          <p className="text-lg sm:text-xl font-bold text-gray-600 max-w-md mx-auto mb-8">Create your first AI agent to start answering visitor questions on your website.</p>
          <button onClick={() => setShowModal(true)} className="brutal-btn bg-brutal-yellow brutal-shadow-sm hover:bg-black hover:text-white text-lg px-8 py-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
          {sites.map((site, i) => (
            <Link href={`/sites/${encodeURIComponent(site.site_id)}`} key={site.id} className={`group block outline-none ${tilt(i)} hover:rotate-0 transition-transform`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="brutal-card brutal-shadow w-full max-w-xl p-6 sm:p-10 bg-white border-[6px] max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-6 sm:mb-8">
              ...
              <button onClick={() => setShowModal(false)} aria-label="Close" className="text-3xl font-black hover:text-brutal-red transition-colors">×</button>
            </div>
            ...
            <form onSubmit={createSite} className="space-y-8">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 pt-4">
                <button type="button" onClick={() => setShowModal(false)} className="brutal-btn w-full sm:flex-1 bg-gray-200 hover:bg-black hover:text-white text-xl py-4">
                  CANCEL
                </button>
                <button type="submit" className="brutal-btn w-full sm:flex-1 bg-brutal-green hover:bg-white text-xl py-4">
                  CREATE
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="brutal-card brutal-shadow w-full max-w-xl p-6 sm:p-10 bg-white border-[6px] max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-6 sm:mb-8">
              ...
              <button onClick={() => setShowLimitModal(false)} aria-label="Close" className="text-3xl font-black hover:text-brutal-red transition-colors">×</button>
            </div>
            ...
            <div className="flex justify-center sm:justify-end pt-8">
              <button onClick={() => setShowLimitModal(false)} className="brutal-btn w-full sm:w-auto bg-black text-white hover:bg-brutal-yellow hover:text-black text-xl py-3 px-8">
                CLOSE
              </button>
            </div>
          </div>
